...
#----------2 degrees filter for estimating derivatives for x, y, z
        ...
        self.log_vision.write('%s %s %s %s %s %s %s %s %s %s\n' \
                % (self.time(), x_cam, y_cam, z_cam, self.x_cam, self.y_cam, self.z_cam, self.x_cam_dot, self.y_cam_dot, self.z_cam_dot))
        self.log_vision.flush()

#---------- Get orientation from LPE -----------------------
...
#------------------------ Generating trajectory ----------------------------
        ...
        if self.time() > 50 and self.circle_time < 7.*self.circle_period:
            ...
            self.circle_time += dt
        elif self.time() > 50+20+7.*self.circle_period and self.circle_time > 6.*self.circle_period and self.spiral_time < 5.*self.circle_period:
            x0 = -0.2
            y0 =  0.2
            R = 0.6
            self.x_ref = x0 + R*sin(2.*pi/self.circle_period*self.spiral_time)
            self.y_ref = y0 + R*cos(2.*pi/self.circle_period*self.spiral_time)
            self.z_ref += -self.zv_ref*1.3/(5.*self.circle_period)*dt
            self.spiral_time += dt
        elif self.time() > 50+20+7.*self.circle_period+5.*self.circle_period+self.spiral_period and self.time() < 50+20+30+7.*self.circle_period+5.*self.circle_period+self.spiral_period:
            self.x_ref = -0.2
            self.y_ref =  0.8
#            self.z_ref =  0.2
#            print self.time(), self.spiral_period
        elif self.time() >= 50+20+30+7.*self.circle_period+5.*self.circle_period+self.spiral_period:
            self.zv_ref *= -1.
            self.spiral_period += 5.*self.circle_period + 30.
            self.spiral_time = 0.
#            print 'AAAAAA', self.spiral_period
