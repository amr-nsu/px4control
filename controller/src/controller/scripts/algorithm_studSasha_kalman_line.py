...
#-------- for line
	self.line_period = 5.
	self.line_time = 0.
	self.Vx = -1.1/self.line_period
	self.Vy = 1.4/self.line_period
...
#-------- for filtering vision position, getting of vision velosities
	...
	self.model_gamma = model_gamma.Model()
...
#        z_gr = self.controller.get_position().z
	...
	if self.gamma_dot > 10:
		self.gamma_dot = 10.
	if self.gamma_dot < -10:
		self.gamma_dot = -10.
       
	y_gr += L*sin(self.gamma)
	vy += L*self.gamma_dot*cos(self.gamma)
	z_gr -= L*cos(self.gamma)
	vz += L*self.gamma_dot*sin(self.gamma) 
	#print 'x_load ',x_load
	#print 'y_load ',y_load
	#print 'z_load ',z_load
	...
	#line	
	if self.time() > 40 and self.line_time < 18*self.line_period:
	    if (self.counter%(self.line_period*100))<1:
	    	#self.Vx = -1.*self.Vx
	  	#self.Vy = -1.*self.Vy
		self.y_ref = -1.*self.y_ref
	...
	M2 = 0.2
        ...
        thrust = (Msum * sqrt(Azz * Azz + Axx * Axx + Ayy*Ayy))/G
